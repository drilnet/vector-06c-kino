...
# Пример вызова: $ perl 'ConvertFilm839.Demidov.S.Disk.2.pl'
...
# ---
	...
	my @array_high_4bit; # Массив для бит из старшей плоскости.

	my $temp; # Временный результат.

	my $low_plane_x;  # Адрес кадра в младшей плоскости (в @array_839_hex).
	my $high_plane_x; # Адрес кадра в старшей плоскости (в @array_839_hex).

	my $file_bmp_length; # Длина в байтах bmp файла.
	my $plane_bmp_x;     # Адрес для bmp файла.
	my $push;            # Для запоминания адреса $plane_bmp_x.
...
# ---
	...
	$filename_839 = 'Disk Images 839/Demidov S. Disk 2.839';
...
# ---
	...
	# Адрес кадра в младшей плоскости (в @array_839_hex).
	$low_plane_x = 0;
	# Адрес кадра в старшей плоскости (в @array_839_hex).
	$high_plane_x = 128 * 2;
	...
	for ($frame = 0; $frame < 164;) # Обработать 164 кадра.
		...
		for ($x = 0; $x < 20;)
			...
			for ($y = 0; $y < 128;)
				...
				# ---
				# Получить полбайта.
				$temp = $array_839_hex[$high_plane_x];
				# В двоичную систему.
				$temp = sprintf('%b', hex $temp);
				# Дополнить нулями слева (если нужно).
				$temp = sprintf("%04d", $temp);
				# Биты в массив.
				@array_high_4bit = split(//, $temp);

				# Обрабатываем бит 7.
				if ($array_high_4bit[0] eq '0' && $array_low_4bit[0] eq '0')
					{
					$array_bmp_hex[$plane_bmp_x] = '00';
					$plane_bmp_x++;
					}
				if ($array_high_4bit[0] eq '0' && $array_low_4bit[0] eq '1')
					{
					$array_bmp_hex[$plane_bmp_x] = '55';
					$plane_bmp_x++;
					}
				if ($array_high_4bit[0] eq '1' && $array_low_4bit[0] eq '0')
					{
					$array_bmp_hex[$plane_bmp_x] = 'aa';
					$plane_bmp_x++;
					}
				if ($array_high_4bit[0] eq '1' && $array_low_4bit[0] eq '1')
					{
				...
				# Обрабатываем бит 6.
				if ($array_high_4bit[1] eq '0' && $array_low_4bit[1] eq '0')
					{
					$array_bmp_hex[$plane_bmp_x] = '00';
					$plane_bmp_x++;
					}
				if ($array_high_4bit[1] eq '0' && $array_low_4bit[1] eq '1')
					{
					$array_bmp_hex[$plane_bmp_x] = '55';
					$plane_bmp_x++;
					}
				if ($array_high_4bit[1] eq '1' && $array_low_4bit[1] eq '0')
					{
					$array_bmp_hex[$plane_bmp_x] = 'aa';
					$plane_bmp_x++;
					}
				if ($array_high_4bit[1] eq '1' && $array_low_4bit[1] eq '1')
					{
					$array_bmp_hex[$plane_bmp_x] = 'ff';
					$plane_bmp_x++;
					}
				...
				# Обрабатываем бит 5.
				if ($array_high_4bit[2] eq '0' && $array_low_4bit[2] eq '0')
					{
					$array_bmp_hex[$plane_bmp_x] = '00';
					$plane_bmp_x++;
					}
				if ($array_high_4bit[2] eq '0' && $array_low_4bit[2] eq '1')
					{
					$array_bmp_hex[$plane_bmp_x] = '55';
					$plane_bmp_x++;
					}
				if ($array_high_4bit[2] eq '1' && $array_low_4bit[2] eq '0')
					{
					$array_bmp_hex[$plane_bmp_x] = 'aa';
					$plane_bmp_x++;
					}
				if ($array_high_4bit[2] eq '1' && $array_low_4bit[2] eq '1')
					{
					$array_bmp_hex[$plane_bmp_x] = 'ff';
					$plane_bmp_x++;
					}
				...
				# Обрабатываем бит 4.
				if ($array_high_4bit[3] eq '0' && $array_low_4bit[3] eq '0')
					{
					$array_bmp_hex[$plane_bmp_x] = '00';
					$plane_bmp_x++;
					}
				if ($array_high_4bit[3] eq '0' && $array_low_4bit[3] eq '1')
					{
					$array_bmp_hex[$plane_bmp_x] = '55';
					$plane_bmp_x++;
					}
				if ($array_high_4bit[3] eq '1' && $array_low_4bit[3] eq '0')
					{
					$array_bmp_hex[$plane_bmp_x] = 'aa';
					$plane_bmp_x++;
					}
				if ($array_high_4bit[3] eq '1' && $array_low_4bit[3] eq '1')
					{
					$array_bmp_hex[$plane_bmp_x] = 'ff';
					$plane_bmp_x++;
					}
		...
		open (InFileBin, ">" . 'Frames - Demidov S. Disk 2/' . $count_frame_zero . '.bmp') or die "Ошибка...";
